...
class Digraph(object):
    ...
    def add_node(self, node):
        ...
        if node not in self.nodes:  # 若该点不在nodes中则加入
            self.nodes.add(node)
            self.edges[node] = []
        else:  # 若在返回错误
    ...
    def add_edge(self, edge):
        ...
        if edge.get_source() in self.nodes and edge.get_destination() in self.nodes:
            self.edges[edge.get_source()].append(edge)  # 若起点终点均在nodes中则加入该边
        else:  # 若不在返回错误
            raise ValueError(
                'Either of the nodes associated with the edge is not in the graph.')
