...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.tiles = {}  # 存储每个title的dirt_mount
    ...
    def get_num_cleaned_tiles(self):
        ...
        num = 0
        for i in self.tiles:
            if self.is_tile_cleaned(i[0], i[1]):
                num += 1
        return num
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        self.direction = random.uniform(0.0, 360.0)  # 随机生成方向
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        """
        return self.width*self.height-len(furniture_tiles)
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        new_position = self.position.get_new_position(
            self.direction, self.speed)  # 生成新位置
        if self.room.is_position_valid(new_position):  # 判断位置是否valid
            self.set_robot_position(new_position)
            self.room.clean_tile_at_position(
                new_position, self.capacity)  # 清理dirty
        else:
            self.direction = random.uniform(0.0, 360.0)  # 随机生成新方向
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for i in range(0, num_trials):
        room = EmptyRoom(width, height, dirt_amount)  # 创建room
        # anim = ps3_visualize.RobotVisualization(num_robots, width, height, False, 0.2)  # 可视化
        robots = []  # 存储机器人的列表
        for j in range(0, num_robots):
            robots.append(robot_type(room, speed, capacity))  # 加入机器人
        while room.get_num_cleaned_tiles() < room.get_num_tiles()*min_coverage:
            for k in robots:
                k.update_position_and_clean()  # 清理房间
                # anim.update(room, robots)
                cleaning_num += 1  # 计算次数
        # anim.done()
    return cleaning_num/(num_trials*num_robots)  # 返回平均时间步骤

# test for optional
# run_simulation(3, 1.0, 1, 20, 10, 3, 1.0, 1, StandardRobot)
...
#       of a 20x20 room?
#   Answer: When the two robot types have the same parameters and the same number of robots,
#     the StandardRobot needs less time-steps to clean the room than the FaultyRobot.
#     For example, when the number of robots is 2, the number of time steps required
#     for StandardRobot is about 1000,while the number of time steps required for
#     FaultyRobot is about 1200.

#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions
#       10x30, 20x15, 25x12, and 50x6?
#   Answer: When the width and height of the room are 10x30 20x15 25x12 50x6,the other parameters are the
#    same,the time steps for the FaultyRobot cleaning the room is more than the StandardRobot,but at 10x30
#    change to 20x15,the number of time steps of the two robots is decreasing,and the number of time
#    steps of the two robot types is increasing during the change of 20x15 to 50x6
#
